...
app.post("/signup", async (req, res) => {
  if (mongoose.connection.readyState !== 1) {
    return res.status(503).json({ message: 'Database unavailable; please try again later', isSignup: false });
  }
  try {
...
function startServer(port) {
  ...
  server.listen(port, () => {
    console.log(`Auction Server Started Successfully on port ${port}`);
  });
}
